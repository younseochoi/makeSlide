...
//    string videoPath = OpenFileDialog();
   ...
   ptContL = Point(width*(1.0/4.0), height*(1.0/4.0));
   ...
   while (1)
   {
      ...
      if (frameNum < 1)
      {
         //질문하기 제목부분과 내용부분 영역설정할것인지, 하면 페이지 설정도 할 것인지
         prevFrame = currFrame.clone();
      ...
      if (index == 2 && flag < 4) {
         ...
         imshow("select title", settingImage);
         String mystr="";
         putText(settingImage, mystr, Point(50, 50), 1, 2, Scalar(100, 200, 20), 2, 8);
         ...
         if (flag == 1) {
            ...
            cout << "드래그 후 아무키 입력" << endl;
            cout << "없다면 드래그하지 않고 아무키 입력" << endl;
            mystr="Drag a Content section";
            // putText(settingImage, mystr, Point(50, 50), 1, 2, Scalar(100, 200, 20), 2, 18);
         ...
         if (flag == 2) {
            ...
            mystr="Is the number of pages written on the PPT?";
            putText(settingImage, mystr, Point(50, 50), 1, 2, Scalar(100, 200, 20), 2, 18);
